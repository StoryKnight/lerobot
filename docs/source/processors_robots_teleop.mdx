...
You can teleoperate an SO-100 or SO-101 follower with a gamepad (PlayStation DualSense, Xbox, Logitech, or any SDL/HID controller) without a leader arm, URDF, or IK solver. Each gamepad axis directly controls one joint:
...
| L1 / R1 (LB / RB) | wrist_roll |
| L2 / R2 (LT / RT) | gripper (close / open) |
| Triangle / Y | End episode (success) |
| Square / X | End episode (failure) |
| Cross / A | Rerecord episode |

### Quick start

```bash
uv sync --extra gamepad --extra feetech
...
On Linux the port is typically `/dev/ttyACM0`; on macOS `/dev/tty.usbmodem...`. If several pads are connected, select one by name:
...
lerobot-teleoperate \
  ...
  --robot.port=COM3 \
  --teleop.type=gamepad \
  --teleop.device_name=dualsense
```

On macOS, pygame does not always see DualSense/Xbox input. Use HID instead:
...
lerobot-teleoperate \
  ...
  --teleop.type=gamepad \
  --teleop.hidapi_fallback=true
```

### How it works

When `make_default_processors()` detects a delta teleop (gamepad or keyboard_ee) paired with a joint-space robot (SO-100/SO-101 follower), it inserts a `MapDeltaActionToJointPositionsStep` into the teleop action processor. This step:
...
Pygame uses SDL's GameController database so a DualSense is remapped to the same axes/buttons as an Xbox pad. The HID fallback understands DualSense/DualShock, Xbox GIP, and Logitech reports.

### Recording datasets

Recording uses the same factory, so the joint-control pipeline is set up automatically:
